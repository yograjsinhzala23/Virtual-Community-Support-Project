...
export class UserEditProfileComponent implements OnInit, OnDestroy {
  ...
  getUserSkill() {
    console.log('Fetching user skills for user ID:', this.loginUserId);
    const getUserSkillSubscribe = this._commonService.getUserSkill(this.loginUserId).subscribe(
      (data: any) => {
        ...
        if (data.result == 1) {
          ...
          this._toast.error({
            detail: 'ERROR',
            summary: data.message || 'Failed to fetch user skills',
      ...
      (err) => {
        ...
        if (err.status === 404) {
          this._toast.error({
            detail: 'ERROR',
            summary: 'User skills not found. Please try again later.',
            duration: APP_CONFIG.toastDuration,
          });
        } else {
          this._toast.error({
            detail: 'ERROR',
            summary: 'Failed to fetch user skills. Please try again later.',
            duration: APP_CONFIG.toastDuration,
          });
        }
      }
    );
    this.unsubscribe.push(getUserSkillSubscribe);
  }
    ...
    const userProfileSubscribe = this._service.getUserProfileDetailById(id).subscribe(
      (data: any) => {
        ...
        if (data.result == 1) {
          ...
          this._toast.error({
            detail: 'ERROR',
            summary: data.message || 'Failed to fetch user profile',
            duration: APP_CONFIG.toastDuration,
          });
        }
      },
      (err) => {
        ...
        if (err.status === 404) {
          this._toast.error({
            detail: 'ERROR',
            summary: 'User profile not found. Please try again later.',
            duration: APP_CONFIG.toastDuration,
          });
        } else {
          this._toast.error({
            detail: 'ERROR',
            summary: 'Failed to fetch user profile. Please try again later.',
            duration: APP_CONFIG.toastDuration,
          });
        }
      }

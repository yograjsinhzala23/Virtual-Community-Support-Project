...
export class AddMissionComponent implements OnInit, OnDestroy {
  ...
  addMissionFormValid() {
    this.addMissionForm = this._fb.group({
      ...
      endDate: [{value: null, disabled: true}, Validators.compose([Validators.required])],
      missionThemeId: [null],
      missionSkillId: [null],
  ...
  setStartDate() {
    ...
    this.addMissionForm.get('endDate')?.enable();
  ...
  getMissionSkillList() {
    const getMissionSkillListSubscription = this._commonService.getMissionSkillList().subscribe((data: any) => {
      if (data.result == 1) {
      ...
      const errorMessage = err?.error?.message || err?.message || 'An error occurred while fetching mission skills'
  ...
  getMissionThemeList() {
    const getMissionThemeListSubscription = this._commonService.getMissionThemeList().subscribe((data: any) => {
      if (data.result == 1) {
      ...
      const errorMessage = err?.error?.message || err?.message || 'An error occurred while fetching mission themes'
      this._toast.error({ detail: "ERROR", summary: errorMessage, duration: APP_CONFIG.toastDuration });
    })
    ...
    if (this.addMissionForm.valid) {
      if (this.imageListArray.length > 0) {
        await this._commonService.uploadImage(this.formData).pipe().toPromise().then((res: any) => {
          ...
          const errorMessage = err?.error?.message || err?.message || 'An error occurred while uploading image'
          this._toast.error({ detail: "ERROR", summary: errorMessage, duration: APP_CONFIG.toastDuration }) 
        });
      }
